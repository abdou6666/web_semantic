...
function App() {
  ...
  const [displayReclamationAndResponses, setDisplayReclamationAndResponses] =
    useState(false);

  const [
    allReclamationsAndResponsesWithClientAndAdminData,
    setAllReclamationsAndResponsesWithClientAndAdminData,
  ] = useState([]);
  const [
    displayAllReclamationsAndResponsesWithClientAndAdminData,
    setDisplayAllReclamationsAndResponsesWithClientAndAdminData,
  ] = useState(false);
  ...
  async function fetchReclamationsAndResponses() {
    const { data } = await axios.get(
      `${base_url}/abdou/getReclamationsAndResponses`
    );
    setAllReclamationAndResponses(data);
  }
  async function fetchClientsReclamationsWithClienDataAndAdminData() {
    const { data } = await axios.get(
      `${base_url}/abdou/getClientsReclamationsWithClienDataAndAdminData`
    );
  ...
  async function searchReclamation() {
    const { data } = await axios.get(
      `${base_url}/abdou/searchReclamation/${searchReclamationInput}`
    );
  ...
  return (
    <div>
      <div>
        <button onClick={() => setDisplayAllBrand((prev) => !prev)}>
          Display All Brands{" "}
        </button>
        {allBrand && displayAllBrand && <Displaybrand allBrand={allBrand} />}
      </div>
      <div>
        <button onClick={() => setDisplayAllProduct((prev) => !prev)}>
          Display All Products{" "}
        </button>
        {allProduct && displayAllProduct && (
          <Displayproduct allProduct={allProduct} />
        )}
      </div>
      <div>
        <button onClick={() => setDisplayAllReclamation((prev) => !prev)}>
          Display All Reclamations{" "}
        </button>
        {allReclamation && displayAllReclamation && (
          <DisplayAbdouReclamation allReclamation={allReclamation} />
        )}
      </div>

      <div>
        <button
          onClick={() => setDisplayReclamationAndResponses((prev) => !prev)}
        >
          Display All Reclamations with responses{" "}
        </button>
        {allReclamationsAndResponses && displayReclamationAndResponses && (
          <DisplayAbdouReclamationWithResponses
            allReclamationsAndResponses={allReclamationsAndResponses}
          />
        )}
      </div>
      ...
      <div>
        <input
          type="text"
          placeholder="search for reclamation"
          value={searchReclamationInput}
          onChange={(e) => setSearchReclamationInput(e.target.value)}
        />
        <button onClick={searchReclamation}>Search</button>
        {showResult && searchReclamationResult && (
          <ShowSearchResult searchReclamationResult={searchReclamationResult} />
        )}
      </div>
    </div>
  );
